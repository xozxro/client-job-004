...
def getTrades(doneTweets):
    ...
    while working_ is False:
        ...
        try:
            ...
            # discord
            headers = {
                'authorization': 'OTE4MDA3MjEwMTA0MTk3MTUw.G_YnPG.aAP9hOD9Hxq7NoW3SM31aa6nymj3Dwpu4gLjgY',
            ...
            try:
                ...
                DD = requests.get('https://discord.com/api/v9/channels/941185612352999496/messages?limit=2',
                                     headers=headers)

                
                DDText = [x['embeds'][0] for x in json.loads(DD.text)]
            ...
            for message in messages:
                ...
                if len(message[1]) > 0:
                    ...
                    for msg in message[1]:
                        ...
                        try:
                            msg = msg['description'].lower()
                        except:
                            continue
                        
                        msg = msg.lower().replace('\n',' ').strip()
                        if 'trim' in msg.lower() or 'bto' in msg.lower() or 'add' in msg.lower() or 'stc' in msg.lower():
                            pass
                        ...
                        if len(msg.split(' ')) >= 3 and ('$' in msg or '@' in msg or '/' in msg):
                            ...
                            if '**DD**' in message[0]:
                                ...
                                try:
                                    ...
                                    ticker = msg.split(' ')[1]
                                    strike = round(float(msg.split(' ')[2][:-1]))
                                    exp = msg.split(' ')[3]
                                    side = msg.split(' ')[2][-1]
                                    try:
                                        price = float(msg.split(' ')[5])
                                    except:
                                        try:
                                            price = float(msg.split(' ')[4].replace('@', ''))
                                        except:
                                            traceback.print_exc()
                                            pass
                                        
                                    if 'trim' in msg:
                                        msg = 'trimmed 1 ' + ticker + ' ' + str(strike) + side + ' ' + exp + ' @' + str(price)
                                    if 'bto' in msg:
                                        msg = 'BTO 1 ' + ticker + ' ' + str(strike) + side + ' ' + exp + ' @' + str(price)
                                    if 'stc' in msg:
                                        msg = 'STC 1 ' + ticker + ' ' + str(strike) + side + ' ' + exp + ' @' + str(price)
